package Miscellaneous;

public class Constants {
    public static final String SSID = "EnriSte_5G";
    //--------------------------------------------------------
    public static final String ON_PATH = "/on";
    public static final String OFF_PATH = "/off";
    public static final String PING_PATH = "/ping";
    public static final String HTTP = "http://";
    public static final String ON_RESPONSE = "on";
    public static final String OFF_RESPONSE = "off";
    //--------------------------------------------------------
    public static final int SHUTTER_TIMER_DELAY_IN_MILLIS = 0;
    public static final int SHUTTER_TIMER_PERIOD_IN_MILLIS = 120000;
    public static final int LIGHTS_TIMER_DELAY_IN_MILLIS = 0;
    public static final int LIGHTS_TIMER_PERIOD_IN_MILLIS = 3000;
    public static final int NIGHT_BEGINNING_TIME = 18;
    public static final int MORNING_BEGINNING_TIME = 7;
    //--------------------------------------------------------
    public static final int WIFI_NEAR_THRESHOLD = -50;
    public static final int WIFI_FAR_THRESHOLD = -60;

}
